...
class TaskErtrackXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, parent_tasks):
        ...
        worksheet.merge_range(4, 2, 4, 13, 'الأعمال التي قامت بتنفيذها الشركة المصرية لتجديد وصيانة خطوط السكك الحديدية',
                              bold_center)
        worksheet.merge_range(5, 2, 5, 14,
                              'أعمال لتعديلات حوش محطة     بخط       قسم      التابع لإدارة هندسة السكك    شهر    2020',
                              bold_center)
        ...
        for idx, task in enumerate(parent_tasks):
            ...
            col += 1
            worksheet.merge_range(row, col, row + 1, col, 'النسبه السابقه المنصرفه', cell_format_header)
            col += 1
            if task.task_header_id:
                ...
                invoice_per_task = self.env['account.move'].search([
                    ('invoice_origin', '=', task.sale_line_id.order_id.name)], limit=1)
                row_task = row
                ...
                for idxx, child_task in enumerate(task.child_ids):
                    ...
                    # النسبه السابقه المنصرفه
                    col += 1
                    worksheet.write(row, col, "%s%s" % (task.task_header_id.previous, "%"), cell_format_row)
                    col += 1
                    if len(percentages) == 11:
            ...
            col += 1
            worksheet.merge_range(row, col, row, last_col+2, '', cell_format_row)
            row += 1
        # worksheet.merge_range(row, 0, row, 2,
        # " = أعمال شحن ناتج الفج للسكك والمفاتيح بعربات برية ونقلها للمقالب العمومية ", bold_right)
