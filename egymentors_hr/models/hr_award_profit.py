...
class HrWorkLocationInherit(models.Model):
    ...
    all_employees_exclude = fields.Boolean("Out Of Awards",
                                           help="If selected this work location will exclude "
                                                "from all action which sellect all companies employees")
...
class HrAwardProfit(models.Model):
    ...
    def generate_all_employee_ids(self):
        ...
        for rec in self:
            if rec.company_id:
                ...
                emp_ids = emp_obj.search([('company_id', '=', rec.company_id.id),
                                          ('contract_id', '!=', False),
                                          ('work_location_id.all_employees_exclude', '=', False),
    ...
    def generate_employee_ids(self):
        ...
        for rec in self:
            ...
            if rec.work_location_id:
                emps_list = rec.line_ids.mapped('employee_id.id')
                emp_ids = emp_obj.search([('contract_id', '!=', False),
                                          ('contract_id.state', '!=', 'cancel'),
