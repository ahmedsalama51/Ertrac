    ...
    def generate_xlsx_report(self, workbook, data, payslips):
        ...
        if rules_without_parent:
            for rule in rules_without_parent:
                ...
                worksheet.write(row, col, "قرشاً", cell_format_header_parent)
                col += 2
        worksheet.merge_range(row-1, col, row, col, 'موقع العمل', cell_format_header_parent)
        col += 1
        ...
        for idx, payslip_id in enumerate(payslips):
            ...
            if rules_without_parent:
                for total in payslip_id.get_free_rule_amount(rules_without_parent):
                    ...
                    col += 2
            # Work Location
            work_location = payslip_id.employee_id.work_location_id and \
                            payslip_id.employee_id.work_location_id.name or '------------'
            worksheet.write(row, col, work_location, cell_format_header_parent)
            col += 1
            # Bank
